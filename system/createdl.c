...
pid32	createdl(
	  ...
	  int32		deadline,   /* requested deadline       */
	...
	)
{
   ...
   // return error if requested deadline is <= 0
    if (deadline <= 0)
    {
        return SYSERR;
    }
	...
	prcount++;
	prptr = &proctab[pid];
	/* get priority of head of ready list */
	struct	procent	*currptr;
	currptr = &proctab[currpid];
	// set priority as the same as first in ready list if at least 10
	if (currptr->prprio >= 10)
	{
		priority = currptr->prprio;
	}
	...
	*pushsp = (unsigned long) (prptr->prstkptr = (char *)saddr);
	...
	// set global variable deadon to true
	if (deadon != TRUE)
	{
		deadon = TRUE;
	}
